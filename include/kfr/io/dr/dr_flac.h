...
//     #include "dr_flac.h"
//
// You can then #include this file in other parts of the program as you would with any other header file. To
// decode audio data, do something like the following:
//
...
//     drflac_uint64 numberOfInterleavedSamplesActuallyRead = drflac_read_s32(pFlac, pFlac->totalSampleCount,
//     pSamples);
//
// The drflac object represents the decoder. It is a transparent type so all the information you need, such as
// the number of channels and the bits per sample, should be directly accessible - just make sure you don't
// change their values. Samples are always output as interleaved signed 32-bit PCM. In the example above a
// native FLAC stream was opened, however dr_flac has seamless support for Ogg encapsulated FLAC streams as
// well.
//
// You do not need to decode the entire stream in one go - you just specify how many samples you'd like at any
// given time and the decoder will give you as many samples as it can, up to the amount requested. Later on
// when you need the next batch of samples, just call it again. Example:
//
//     while (drflac_read_s32(pFlac, chunkSize, pChunkSamples) > 0) {
//         do_something();
//     }
//
// You can seek to a specific sample with drflac_seek_to_sample(). The given sample is based on interleaving.
// So for example, if you were to seek to the sample at index 0 in a stereo stream, you'll be seeking to the
// first sample of the left channel. The sample at index 1 will be the first sample of the right channel. The
...
//
//
// You can read samples as signed 16-bit integer and 32-bit floating-point PCM with the *_s16() and *_f32()
// family of APIs respectively, but note that these should be considered lossy.
//
//
// If you need access to metadata (album art, etc.), use drflac_open_with_metadata(),
// drflac_open_file_with_metdata() or drflac_open_memory_with_metadata(). The rationale for keeping these APIs
// separate is that they're slightly slower than the normal versions and also just a little bit harder to use.
//
// dr_flac reports metadata to the application through the use of a callback, and every metadata block is
// reported before drflac_open_with_metdata() returns.
//
//
// The main opening APIs (drflac_open(), etc.) will fail if the header is not present. The presents a problem
// in certain scenarios such as broadcast style streams like internet radio where the header may not be
// present because the user has started playback mid-stream. To handle this, use the relaxed APIs:
// drflac_open_relaxed() and drflac_open_with_metadata_relaxed().
//
// It is not recommended to use these APIs for file based streams because a missing header would usually
// indicate a corrupted or perverse file. In addition, these APIs can take a long time to initialize because
// they may need to spend a lot of time finding the first frame.
//
//
//
// OPTIONS
...
//   Defines the size of the internal buffer to store data from onRead(). This buffer is used to reduce the
//   number of calls back to the client for more data. Larger values means more memory, but better
//   performance. My tests show diminishing returns after about 4KB (which is the default). Consider reducing
//   this if you have a very efficient implementation of onRead(), or increase it if it's very inefficient.
...
//   Disables SIMD optimizations (SSE on x86/x64 architectures). Use this if you are having compatibility
...
// - Audio data is output as signed 32-bit PCM, regardless of the bits per sample the FLAC stream is encoded
// as.
...
// size of that buffer. Larger values means more speed, but also more memory. In my testing there is
// diminishing returns after about 4KB, but you can fiddle with this to suit your own needs. Must be a
// multiple of 8.
...
// Packing is important on this structure because we map this directly to the raw data within the SEEKTABLE
// metadata block.
#pragma pack(2)
    typedef struct
    {
...
#pragma pack()
    ...
    //
    // The offset will never be negative. Whether or not it is relative to the beginning or current position
    // is determined by the "origin" parameter which will be either drflac_seek_origin_start or
    // drflac_seek_origin_current.
    ...
    typedef struct
    {
        ...
        // The number of unaligned bytes in the L2 cache. This will always be 0 until the end of the stream is
        // hit. At the end of the stream there will be a number of bytes that don't cleanly fit in an L1 cache
        // line, so we use this variable to know whether or not the bistreamer needs to run on a slower path
        // to read those last bytes. This will never be more than sizeof(drflac_cache_t).
        ...
        // The number of bits that have been consumed by the cache. This is used to determine how many valid
        // bits are remaining.
        drflac_uint32 consumedBits;

        // The cached data which was most recently read from the client. There are two levels of cache. Data
        // flows as such: Client -> L2 -> L1. The L2 -> L1 movement is aligned and runs on a fast path in just
        // a few instructions.
        drflac_cache_t cacheL2[DR_FLAC_BUFFER_SIZE / sizeof(drflac_cache_t)];
        drflac_cache_t cache;

        // CRC-16. This is updated whenever bits are read from the bit stream. Manually set this to 0 to reset
        // the CRC. For FLAC, this is reset to 0 at the beginning of each frame.
        drflac_uint16 crc16;
        drflac_cache_t crc16Cache; // A cache for optimizing CRC calculations. This is filled when when the L1
                                   // cache is reloaded.
        drflac_uint32 crc16CacheIgnoredBytes; // The number of bytes to ignore when updating the CRC-16 from
                                              // the CRC-16 cache.
    } drflac_bs;
    ...
    typedef struct
    {
        ...
        // The number of bits per sample for this subframe. This is not always equal to the current frame's
        // bit per sample because an extra bit is required for side channels when interchannel decorrelation
        // is being used.
        drflac_uint32 bitsPerSample;

        // A pointer to the buffer containing the decoded samples in the subframe. This pointer is an offset
        // from drflac::pExtraData. Note that it's a signed 32-bit integer for each value.
        drflac_int32* pDecodedSamples;
    } drflac_subframe;

    typedef struct
    {
        // If the stream uses variable block sizes, this will be set to the index of the first sample. If
        // fixed block sizes are used, this will always be set to 0.
        drflac_uint64 sampleNumber;

        // If the stream uses fixed block sizes, this will be set to the frame number. If variable block sizes
        // are used, this will always be 0.
    ...
    typedef struct
    {
        ...
        // The number of samples left to be read in this frame. This is initially set to the block size
        // multiplied by the channel count. As samples are read, this will be decremented. When it reaches 0,
        // the decoder will see this frame as fully consumed and load the next frame.
        drflac_uint32 samplesRemaining;

        // The list of sub-frames within the frame. There is one sub-frame for each channel, and there's a
    ...
    typedef struct
    {
        ...
        // The number of channels. This will be set to 1 for monaural streams, 2 for stereo, etc. Maximum 8.
        // This is set based on the value specified in the STREAMINFO block.
        drflac_uint8 channels;

        // The bits per sample. Will be set to something like 16, 24, etc.
        drflac_uint8 bitsPerSample;

        // The maximum block size, in samples. This number represents the number of samples in each channel
        // (not combined).
        drflac_uint16 maxBlockSize;

        // The total number of samples making up the stream. This includes every channel. For example, if the
        // stream has 2 channels, with each channel having a total of 4096, this value will be set to 2*4096 =
        // 8192. Can be 0 in which case it's still a valid stream, but just means the total sample count is
        // unknown. Likely the case with streams like internet radio.
        drflac_uint64 totalSampleCount;

        // The container type. This is set based on whether or not the decoder was opened from a native or Ogg
        // stream.
        ...
        // Variable length extra data. We attach this to the end of the object so we can avoid unnecessary
        // mallocs.
    ...
    // pUserData [in, optional] A pointer to application defined data that will be passed to onRead and
    // onSeek.
    //
    ...
    // encapsulation also works with multiplexed streams which basically means it can play FLAC encoded audio
    // tracks in videos.
    //
    ...
    // (Native or Ogg). Do not set this to drflac_container_unknown - that is for internal use only.
    //
    // Opening in relaxed mode will continue reading data from onRead until it finds a valid frame. If a frame
    // is never found it will continue forever. To abort, force your onRead callback to return 0, which
    // dr_flac will use as an indicator that the end of the stream was found.
    drflac* drflac_open_relaxed(drflac_read_proc onRead, drflac_seek_proc onSeek, drflac_container container,
                                void* pUserData);
    ...
    // pUserData [in, optional] A pointer to application defined data that will be passed to onRead, onSeek
    // and onMeta.
    //
    // Returns a pointer to an object representing the decoder.
    //
    // Close the decoder with drflac_close().
    //
    // This is slower than drflac_open(), so avoid this one if you don't need metadata. Internally, this will
    // do a DRFLAC_MALLOC() and DRFLAC_FREE() for every metadata block except for STREAMINFO and PADDING
    // blocks.
    //
    // The caller is notified of the metadata via the onMeta callback. All metadata blocks will be handled
    // before the function returns.
    //
    // The STREAMINFO block must be present for this to succeed. Use drflac_open_with_metadata_relaxed() to
    // open a FLAC stream where the header may not be present.
    //
    // Note that this will behave inconsistently with drflac_open() if the stream is an Ogg encapsulated
    // stream and a metadata block is corrupted. This is due to the way the Ogg stream recovers from corrupted
    // pages. When drflac_open_with_metadata() is being used, the open routine will try to read the contents
    // of the metadata block, whereas drflac_open() will simply seek past it (for the sake of efficiency).
    // This inconsistency can result in different samples being returned depending on whether or not the
    // stream is being opened with metadata.
    //
    // See also: drflac_open_file_with_metadata(), drflac_open_memory_with_metadata(), drflac_open(),
    // drflac_close()
    drflac* drflac_open_with_metadata(drflac_read_proc onRead, drflac_seek_proc onSeek,
                                      drflac_meta_proc onMeta, void* pUserData);

    // The same as drflac_open_with_metadata(), except attempts to open the stream even when a header block is
    // not present.
    //
    // See also: drflac_open_with_metadata(), drflac_open_relaxed()
    drflac* drflac_open_with_metadata_relaxed(drflac_read_proc onRead, drflac_seek_proc onSeek,
    ...
    // number of samples seeked.
    //
    // Note that this should be considered lossy due to the nature of floating point numbers not being able to
    // exactly represent every possible number.
    ...
    //
    // The sample index is based on interleaving. In a stereo stream, for example, the sample at index 0 is
    // the first sample in the left channel; the sample at index 1 is the first sample on the right channel,
    // and so on.
    //
    // When seeking, you will likely want to ensure it's rounded to a multiple of the channel count. You can
    // do this with something like drflac_seek_to_sample(pFlac, (mySampleIndex + (mySampleIndex %
    // pFlac->channels)))
    ...
    // will restrict the number of files a process can have open at any given time, so keep this mind if you
    // have many decoders open at the same time.
    //
    ...
    //
    // This does not create a copy of the data. It is up to the application to ensure the buffer remains valid
    ...
    // Look at the documentation for drflac_open_with_metadata() for more information on how metadata is
    // handled.
    drflac* drflac_open_memory_with_metadata(const void* data, size_t dataSize, drflac_meta_proc onMeta,
                                             void* pUserData);

    //// High Level APIs ////

    // Opens a FLAC stream from the given callbacks and fully decodes it in a single operation. The return
    // value is a pointer to the sample data as interleaved signed 32-bit PCM. The returned data must be freed
    // with DRFLAC_FREE().
    //
    ...
    drflac_int32* drflac_open_and_decode_s32(drflac_read_proc onRead, drflac_seek_proc onSeek,
                                             void* pUserData, unsigned int* channels,
                                             unsigned int* sampleRate, drflac_uint64* totalSampleCount);

    // Same as drflac_open_and_decode_s32(), except returns signed 16-bit integer samples.
    drflac_int16* drflac_open_and_decode_s16(drflac_read_proc onRead, drflac_seek_proc onSeek,
                                             void* pUserData, unsigned int* channels,
                                             unsigned int* sampleRate, drflac_uint64* totalSampleCount);

    // Same as drflac_open_and_decode_s32(), except returns 32-bit floating-point samples.
    float* drflac_open_and_decode_f32(drflac_read_proc onRead, drflac_seek_proc onSeek, void* pUserData,
                                      unsigned int* channels, unsigned int* sampleRate,
                                      drflac_uint64* totalSampleCount);

#ifndef DR_FLAC_NO_STDIO
    // Same as drflac_open_and_decode_s32() except opens the decoder from a file.
    drflac_int32* drflac_open_and_decode_file_s32(const char* filename, unsigned int* channels,
                                                  unsigned int* sampleRate, drflac_uint64* totalSampleCount);

    // Same as drflac_open_and_decode_file_s32(), except returns signed 16-bit integer samples.
    drflac_int16* drflac_open_and_decode_file_s16(const char* filename, unsigned int* channels,
                                                  unsigned int* sampleRate, drflac_uint64* totalSampleCount);

    // Same as drflac_open_and_decode_file_f32(), except returns 32-bit floating-point samples.
    float* drflac_open_and_decode_file_f32(const char* filename, unsigned int* channels,
                                           unsigned int* sampleRate, drflac_uint64* totalSampleCount);
#endif

    // Same as drflac_open_and_decode_s32() except opens the decoder from a block of memory.
    drflac_int32* drflac_open_and_decode_memory_s32(const void* data, size_t dataSize, unsigned int* channels,
                                                    unsigned int* sampleRate,
                                                    drflac_uint64* totalSampleCount);

    // Same as drflac_open_and_decode_memory_s32(), except returns signed 16-bit integer samples.
    drflac_int16* drflac_open_and_decode_memory_s16(const void* data, size_t dataSize, unsigned int* channels,
                                                    unsigned int* sampleRate,
                                                    drflac_uint64* totalSampleCount);

    // Same as drflac_open_and_decode_memory_s32(), except returns 32-bit floating-point samples.
    float* drflac_open_and_decode_memory_f32(const void* data, size_t dataSize, unsigned int* channels,
                                             unsigned int* sampleRate, drflac_uint64* totalSampleCount);
    ...
    void drflac_init_vorbis_comment_iterator(drflac_vorbis_comment_iterator* pIter,
                                             drflac_uint32 commentCount, const void* pComments);

    // Goes to the next vorbis comment in the given iterator. If null is returned it means there are no more
    // comments. The returned string is NOT null terminated.
...
// Packing is important on this structure because we map this directly to the raw data within the CUESHEET
// metadata block.
#pragma pack(4)
    typedef struct
    {
...
#pragma pack()
    ...
    void drflac_init_cuesheet_track_iterator(drflac_cuesheet_track_iterator* pIter, drflac_uint32 trackCount,
                                             const void* pTrackData);

    // Goes to the next cuesheet track in the given iterator. If DRFLAC_FALSE is returned it means there are
    // no more comments.
    drflac_bool32 drflac_next_cuesheet_track(drflac_cuesheet_track_iterator* pIter,
...
{
// It looks like the -fPIC option uses the ebx register which GCC complains about. We can work around this by
// just using a different register, the specific register of which I'm letting the compiler decide on. The "k"
// prefix is used to specify a 32-bit register. The {...} syntax is for supporting different assembly
// dialects.
//
...
static DRFLAC_INLINE drflac_uint16 drflac_crc16__32bit(drflac_uint16 crc, drflac_uint32 data,
                                                       drflac_uint32 count)
{
...
static DRFLAC_INLINE drflac_uint16 drflac_crc16__64bit(drflac_uint16 crc, drflac_uint64 data,
                                                       drflac_uint32 count)
{
    drflac_assert(count <= 64);
    ...
    case 8:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0xFF00000000000000 << leftoverBits)) >>
                                               (56 + leftoverBits)));
    case 7:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x00FF000000000000 << leftoverBits)) >>
                                               (48 + leftoverBits)));
    case 6:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x0000FF0000000000 << leftoverBits)) >>
                                               (40 + leftoverBits)));
    case 5:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x000000FF00000000 << leftoverBits)) >>
                                               (32 + leftoverBits)));
    case 4:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x00000000FF000000 << leftoverBits)) >>
                                               (24 + leftoverBits)));
    case 3:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x0000000000FF0000 << leftoverBits)) >>
                                               (16 + leftoverBits)));
    case 2:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x000000000000FF00 << leftoverBits)) >>
                                               (8 + leftoverBits)));
    case 1:
        crc = drflac_crc16_byte(crc,
                                (drflac_uint8)((data & ((drflac_uint64)0x00000000000000FF << leftoverBits)) >>
                                               (0 + leftoverBits)));
    case 0:
        if (leftoverBits > 0)
            crc = (crc << leftoverBits) ^
...
//
// This uses a 32- or 64-bit bit-shifted cache - as bits are read, the cache is shifted such that the first
// valid bit is sitting on the most significant bit. It uses the notion of an L1 and L2 cache (borrowed from
// CPU architecture), where the L1 cache is a 32- or 64-bit unsigned integer (depending on whether or not a
// 32- or 64-bit build is being compiled) and the L2 is an array of "cache lines", with each cache line being
// the same size as the L1. The L2 is a buffer of about 4KB and is where data from onRead() is read into.
...
static DRFLAC_INLINE drflac_uint16 drflac__flush_crc16(drflac_bs* bs)
{
    ...
    // The bits that were read from the L1 cache need to be accumulated. The number of bytes needing to be
    // accumulated is determined by the number of bits that have been consumed.
    ...
    else
    {
        ...
        // The bits that we just accumulated should never be accumulated again. We need to keep track of how
        // many bytes were accumulated so we can handle that later.
        bs->crc16CacheIgnoredBytes = bs->consumedBits >> 3;
    }
...
static DRFLAC_INLINE drflac_bool32 drflac__reload_l1_cache_from_l2(drflac_bs* bs)
{
    ...
    {
        return DRFLAC_FALSE; // If we have any unaligned bytes it means there's no more aligned bytes left in
                             // the client.
    }
    ...
    // means we've just reached the end of the file. We need to move the valid data down to the end of the
    // buffer and adjust the index of the next line accordingly. Also keep in mind that the L2 cache must be
    // aligned to the size of the L1 so we'll need to seek backwards by any misaligned bytes.
...
static drflac_bool32 drflac__reload_cache(drflac_bs* bs)
{
    ...
    // If we get here it means we have failed to load the L1 cache from the L2. Likely we've just reached the
    // end of the stream and the last few bytes did not meet the alignment requirements for the L2 cache. In
    // this case we need to fall back to a slower path and read the data from the unaligned cache.
    ...
    bs->cache &= DRFLAC_CACHE_L1_SELECTION_MASK(
        DRFLAC_CACHE_L1_BITS_REMAINING(bs)); // <-- Make sure the consumed bits are always set to zero. Other
                                             // parts of the library depend on this property.
    bs->unalignedByteCount = 0; // <-- At this point the unaligned bytes have been moved into the cache and we
                                // thus have no more unaligned bytes.
                                                       ...
                                                       drflac_uint32* pResultOut)
{
    ...
    {
        // If we want to load all 32-bits from a 32-bit cache we need to do it slightly differently because we
        // can't do a 32-bit shift on a 32-bit integer. This will never be the case on 64-bit caches, so we
        // can have a slightly more optimal solution for this.
#ifdef DRFLAC_64BIT
    ...
    {
        // It straddles the cached data. It will never cover more than the next chunk. We just read the number
        // in two parts and combine them.
...
static drflac_bool32 drflac__seek_bits(drflac_bs* bs, size_t bitsToSeek)
{
    ...
    {
        // It straddles the cached data. This function isn't called too frequently so I'm favouring simplicity
        // here.
...
// This function moves the bit streamer to the first bit after the sync code (bit 15 of the of the frame
// header). It will also update the CRC-16.
...
{
    // This function assumes at least one bit is set. Checking for 0 needs to be done at a higher level,
...
static drflac_bool32 drflac__seek_to_byte(drflac_bs* bs, drflac_uint64 offsetFromStart)
{
    ...
    // Seeking from the start is not quite as trivial as it sounds because the onSeek callback takes a signed
    // 32-bit integer (which is intentional because it simplifies the implementation of the onSeek callbacks),
    // however offsetFromStart is unsigned 64-bit. To resolve we just need to do an initial seek from the
    // start, and then a series of offset seeks to make up the remainder.
...
// precision. It's safe to assume this will be slower on 32-bit platforms so we use a more optimal solution
// when the bits per sample is <=16.
                                                                  ...
                                                                  drflac_int32* pDecodedSamples)
{
    ...
    // VC++ optimizes this to a single jmp instruction, but only the 64-bit build. The 32-bit build generates
    // less efficient code for some reason. The ugly version above is faster so we'll just switch between the
    // two depending on the target platform.
#ifdef DRFLAC_64BIT
...
static DRFLAC_INLINE drflac_bool32 drflac__read_rice_parts(drflac_bs* bs, drflac_uint8 riceParam,
                                                           drflac_uint32* pZeroCounterOut,
                                                           drflac_uint32* pRiceParamPartOut)
{
    drflac_assert(riceParam >
                  0); // <-- riceParam should never be 0. drflac__read_rice_parts__param_equals_zero() should
                      // be used instead for this case.
    ...
    if (riceLength < DRFLAC_CACHE_L1_BITS_REMAINING(bs))
    {
        riceParamPart = (drflac_uint32)((bs->cache & (riceParamMask >> setBitOffsetPlus1)) >>
                                        DRFLAC_CACHE_L1_SELECTION_SHIFT(bs, riceLength));
    ...
    else
    {
        ...
        bs->cache <<=
            setBitOffsetPlus1 & (DRFLAC_CACHE_L1_SIZE_BITS(bs) -
                                 1); // <-- Equivalent to "if (setBitOffsetPlus1 <
                                     // DRFLAC_CACHE_L1_SIZE_BITS(bs)) { bs->cache <<= setBitOffsetPlus1; }"

        // It straddles the cached data. It will never cover more than the next chunk. We just read the number
        // in two parts and combine them.
        drflac_uint32 bitCountLo = bs->consumedBits - DRFLAC_CACHE_L1_SIZE_BITS(bs);
        drflac_cache_t resultHi  = DRFLAC_CACHE_L1_SELECT_AND_SHIFT(
            bs, riceParam); // <-- Use DRFLAC_CACHE_L1_SELECT_AND_SHIFT_SAFE() if ever this function allows
                            // riceParam=0.
...
static DRFLAC_INLINE drflac_bool32 drflac__read_rice_parts__param_equals_zero(
    drflac_bs* bs, drflac_uint32* pZeroCounterOut, drflac_uint32* pRiceParamPartOut)
{
...
static drflac_bool32 drflac__decode_samples_with_residual__rice__simple(
...
static drflac_bool32 drflac__decode_samples_with_residual__rice__param_equals_zero(
    drflac_bs* bs, drflac_uint32 bitsPerSample, drflac_uint32 count, drflac_uint32 order, drflac_int32 shift,
                                                                ...
                                                                drflac_int32* pSamplesOut)
{
    ...
    if (riceParam != 0)
    {
        return drflac__decode_samples_with_residual__rice__simple(bs, bitsPerSample, count, riceParam, order,
                                                                  shift, coefficients, pSamplesOut);
    }
    else
    {
        return drflac__decode_samples_with_residual__rice__param_equals_zero(
            bs, bitsPerSample, count, order, shift, coefficients, pSamplesOut);
    }
#endif
}

// Reads and seeks past a string of residual values as Rice codes. The decoder should be sitting on the first
// bit of the Rice codes.
...
// Reads and decodes the residual for the sub-frame the decoder is currently sitting on. This function should
// be called when the decoder is sitting at the very start of the RESIDUAL block. The first <order> residuals
// will be ignored. The <blockSize> and <order> parameters are used to determine how many residual values need
// to be decoded.
                                                          ...
                                                          drflac_int32* pDecodedSamples)
{
    ...
    drflac_assert(pDecodedSamples != NULL); // <-- Should we allow NULL, in which case we just seek past the
                                            // residual rather than do a full decode?
    ...
    for (;;)
    {
        ...
        if (riceParam != 0xFF)
        {
            if (!drflac__decode_samples_with_residual__rice(bs, bitsPerSample, samplesInPartition, riceParam,
...
// Reads and seeks past the residual for the sub-frame the decoder is currently sitting on. This function
// should be called when the decoder is sitting at the very start of the RESIDUAL block. The first <order>
// residuals will be set to 0. The <blockSize> and <order> parameters are used to determine how many residual
                                                   ...
                                                   drflac_int32* pDecodedSamples)
{
    ...
    if (!drflac__decode_samples_with_residual(bs, bitsPerSample, blockSize, lpcOrder, 0,
...
static drflac_bool32 drflac__read_next_frame_header(drflac_bs* bs, drflac_uint8 streaminfoBitsPerSample,
                                                    drflac_frame_header* header)
{
    drflac_assert(bs != NULL);
                                             ...
                                             drflac_int32* pDecodedSamplesOut)
{
    ...
    {
        drflac__decode_samples__constant(bs, frame->header.blockSize, pSubframe->bitsPerSample,
                                         pSubframe->pDecodedSamples);
    }
    break;

    case DRFLAC_SUBFRAME_VERBATIM:
    {
        drflac__decode_samples__verbatim(bs, frame->header.blockSize, pSubframe->bitsPerSample,
                                         pSubframe->pDecodedSamples);
    }
    break;
    ...
    {
        drflac__decode_samples__fixed(bs, frame->header.blockSize, pSubframe->bitsPerSample,
                                      pSubframe->lpcOrder, pSubframe->pDecodedSamples);
    }
    break;

    case DRFLAC_SUBFRAME_LPC:
    {
        drflac__decode_samples__lpc(bs, frame->header.blockSize, pSubframe->bitsPerSample,
                                    pSubframe->lpcOrder, pSubframe->pDecodedSamples);
    }
    break;
...
static drflac_result drflac__decode_frame(drflac* pFlac)
{
    ...
    {
        if (!drflac__decode_subframe(&pFlac->bs, &pFlac->currentFrame, i,
...
static void drflac__get_current_frame_sample_range(drflac* pFlac, drflac_uint64* pFirstSampleInFrameOut,
                                                   drflac_uint64* pLastSampleInFrameOut)
{
    ...
    drflac_uint64 lastSampleInFrame =
        firstSampleInFrame + (pFlac->currentFrame.header.blockSize * channelCount);
...
{
    // This function should only ever be called while the decoder is sitting on the first byte past the
    // FRAME_HEADER section.
    drflac_assert(pFlac != NULL);
...
static drflac_bool32 drflac__seek_to_sample__brute_force(drflac* pFlac, drflac_uint64 sampleIndex)
{
    ...
    // If we are seeking forward we start from the current position. Otherwise we need to start all the way
    ...
    if (sampleIndex >= pFlac->currentSample)
    {
        ...
        {
            if (!drflac__read_next_frame_header(&pFlac->bs, pFlac->bitsPerSample,
    ...
    // iterate over each frame and inspect its header. If based on the header we can determine that the frame
    // contains the sample, we do a full decode of that frame.
    for (;;)
    {
            ...
            if (!isMidFrame)
            {
                ...
                if (result == DRFLAC_SUCCESS)
                {
                    ...
                    return drflac_read_s32(pFlac, samplesToDecode, NULL) ==
                           samplesToDecode; // <-- If this fails, something bad has happened (it should never
                                            // fail).
                }
                else
                {
        ...
        else
        {
            ...
            else
            {
                ...
                // instead of with drflac__seek_to_next_frame() which only works if the decoder is sitting on
                // the byte just after the frame header.
...
static drflac_bool32 drflac__seek_to_sample__seek_table(drflac* pFlac, drflac_uint64 sampleIndex)
{
    ...
    // At this point we should have found the seekpoint closest to our sample. If we are seeking forward and
    // the closest seekpoint is _before_ the current sample, we just seek forward from where we are. Otherwise
    // we start seeking from the seekpoint's first sample.
    drflac_uint64 runningSampleCount;
    if ((sampleIndex >= pFlac->currentSample) &&
        (pFlac->pSeekpoints[iClosestSeekpoint].firstSample * pFlac->channels <= pFlac->currentSample))
    {
    ...
    else
    {
        ...
        if (!drflac__seek_to_byte(&pFlac->bs,
                                  pFlac->firstFramePos + pFlac->pSeekpoints[iClosestSeekpoint].frameOffset))
        {
            return DRFLAC_FALSE;
        }
    ...
    for (;;)
    {
        ...
        if (sampleIndex < (runningSampleCount + sampleCountInThisFrame))
        {
            // The sample should be in this frame. We need to fully decode it, but if it's an invalid frame (a
            // CRC mismatch) we need to pretend it never existed and keep iterating.
            drflac_uint64 samplesToDecode = sampleIndex - runningSampleCount;
...
drflac_bool32 drflac__read_streaminfo(drflac_read_proc onRead, void* pUserData,
                                      drflac_streaminfo* pStreamInfo)
{
    ...
    pStreamInfo->bitsPerSample =
        (drflac_uint8)((importantProps & (drflac_uint64)0x000001F000000000) >> 36) + 1;
    pStreamInfo->totalSampleCount =
        (importantProps & (drflac_uint64)0x0000000FFFFFFFFF) * pStreamInfo->channels;
    drflac_copy_memory(pStreamInfo->md5, md5, sizeof(md5));

    return DRFLAC_TRUE;
}

drflac_bool32 drflac__read_and_decode_metadata(drflac_read_proc onRead, drflac_seek_proc onSeek,
...
{
    // We want to keep track of the byte position in the stream of the seektable. At the time of calling this
    // function we know that we'll be sitting on byte 42.
    ...
    for (;;)
    {
        ...
        case DRFLAC_METADATA_BLOCK_TYPE_SEEKTABLE:
        {
            ...
            if (onMeta)
            {
                ...
                // Endian swap.
                for (drflac_uint32 iSeekpoint = 0; iSeekpoint < metadata.data.seektable.seekpointCount;
                     ++iSeekpoint)
                {
        ...
        case DRFLAC_METADATA_BLOCK_TYPE_VORBIS_COMMENT:
        {
            ...
            if (onMeta)
            {
                ...
                // Need space for 'commentCount' comments after the block, which at minimum is a drflac_uint32
                // per comment
                ...
                for (drflac_uint32 i = 0; i < metadata.data.vorbis_comment.commentCount; ++i)
                {
                    ...
                    const drflac_uint32 commentLength =
                        drflac__le2host_32(*(const drflac_uint32*)pRunningData);
                    pRunningData += 4;
        ...
        case DRFLAC_METADATA_BLOCK_TYPE_CUESHEET:
        {
            ...
            if (onMeta)
            {
                ...
                for (drflac_uint8 i = 0; i < metadata.data.cuesheet.trackCount; ++i)
                {
                    ...
                    pRunningData += 35;
                    const drflac_uint8 indexCount = pRunningData[0];
                    pRunningData += 1;
        ...
        default:
        {
            // It's an unknown chunk, but not necessarily invalid. There's a chance more metadata blocks might
            // be defined later on, so we can at the very least report the chunk to the application and let it
        ...
        // If we're not handling metadata, just skip over the block. If we are, it will have been handled
        // earlier in the switch statement above.
...
drflac_bool32 drflac__init_private__native(drflac_init_info* pInit, drflac_read_proc onRead,
                                           ...
                                           void* pUserDataMD, drflac_bool32 relaxed)
{
    ...
    if (blockType != DRFLAC_METADATA_BLOCK_TYPE_STREAMINFO || blockSize != 34)
    {
        ...
        else
        {
            ...
            {
                return DRFLAC_FALSE; // Failed to initialize because the first frame depends on the STREAMINFO
    ...
    else
    {
        ...
        pInit->maxBlockSize =
            streaminfo.maxBlockSize; // Don't care about the min block size - only the max (used for
...
drflac_result drflac_ogg__read_page_header_after_capture_pattern(drflac_read_proc onRead, void* pUserData,
    ...
    for (;;)
    {
        if (drflac_ogg__is_capture_pattern(id))
        {
            *pCRC32 = DRFLAC_OGG_CAPTURE_PATTERN_CRC32;
            ...
            drflac_result result = drflac_ogg__read_page_header_after_capture_pattern(
...
// bitstream. It works in three general stages: Ogg Physical Bitstream -> Ogg/FLAC Logical Bitstream -> FLAC
// Native Bitstream. dr_flac is designed in such a way that the core sections assume everything is delivered
// in native format. Therefore, for each encapsulation type dr_flac is supporting there needs to be a layer
// sitting on top of the onRead and onSeek callbacks that ensures the bits read from the physical Ogg
// bitstream are converted and delivered in native FLAC format.
typedef struct
{
    ...
    drflac_uint64 currentBytePos; // The position of the byte we are sitting on in the physical byte stream.
                                  // Used for efficient seeking.
    drflac_uint64 firstBytePos; // The position of the first byte in the physical bitstream. Points to the
                                // start of the "OggS" identifier of the FLAC bos page.
    drflac_uint32 serialNumber; // The serial number of the FLAC audio pages. This is determined by the
                                // initial header page that was read during initialization.
                                                  ...
                                                  drflac_ogg_crc_mismatch_recovery recoveryMethod)
{
    ...
    for (;;)
    {
        ...
        // We need to read the entire page and then do a CRC check on it. If there's a CRC mismatch we need to
        // skip this page.
...
drflac_bool32 drflac_ogg__seek_to_sample(drflac* pFlac, drflac_uint64 sampleIndex)
{
    ...
    // We found the page that that is closest to the sample, so now we need to find it. The first thing to do
    // is seek to the start of that page. In the loop above we checked that it was a fresh page which means
    // this page is also the start of a new frame. This property means that after we've seeked to the page we
    // can immediately start looping over frames until we find the one containing the target sample.
    ...
    // At this point we'll be sitting on the first byte of the frame header of the first frame in the page. We
    // just keep looping over these frames until we find the one containing the sample we're after.
    ...
    for (;;)
    {
        ...
        //
        // Both of these options have their own pros and cons. Using the native FLAC decoder is slower because
        // it needs to do a full decode of the frame. Using Ogg's framing system is faster, but more
        // complicated and involves some code duplication for the decoding of frame headers.
        //
        // Another thing to consider is that using the Ogg framing system will perform direct seeking of the
        // physical Ogg bitstream. This is important to consider because it means we cannot read data from the
        // drflac_bs object using the standard drflac__*() APIs because that will read in extra data for its
        // own internal caching which in turn breaks the positioning of the read pointer of the physical Ogg
        // bitstream. Therefore, anything that would normally be read using the native FLAC decoding APIs,
        // such as drflac__read_next_frame_header(), need to be re-implemented so as to avoid the use of the
        // drflac_bs object.
        //
        ...
        if (sampleIndex < (runningSampleCount + sampleCountInThisFrame))
        {
            ...
            {
                // The frame is valid. We just need to skip over some samples to ensure it's sample-exact.
                drflac_uint64 samplesToDecode =
                    (size_t)(sampleIndex - runningSampleCount); // <-- Safe cast because the maximum number of
...
drflac_bool32 drflac__init_private__ogg(drflac_init_info* pInit, drflac_read_proc onRead,
                                        drflac_seek_proc onSeek, drflac_meta_proc onMeta, void* pUserData,
                                        void* pUserDataMD, drflac_bool32 relaxed)
{
    ...
    // We'll get here if the first 4 bytes of the stream were the OggS capture pattern, however it doesn't
    // necessarily mean the stream includes FLAC encoded audio. To check for this we need to scan the
    // beginning-of-stream page markers and check if any match the FLAC specification. Important to keep in
    // mind that the stream may be multiplexed.
    drflac_ogg_page_header header;
        ...
        { // 51 = the lacing value of the FLAC header packet.
            ...
            if (packetType == 0x7F)
            {
                ...
                if (sig[0] == 'F' && sig[1] == 'L' && sig[2] == 'A' && sig[3] == 'C')
                {
                    ...
                    // this because we're going to be handling it in a generic way based on the serial number
                    // and packet types.
                    ...
                    if (sig[0] == 'f' && sig[1] == 'L' && sig[2] == 'a' && sig[3] == 'C')
                    {
                        ...
                        if (blockType != DRFLAC_METADATA_BLOCK_TYPE_STREAMINFO || blockSize != 34)
                        {
                            return DRFLAC_FALSE; // Invalid block type. First block must be the STREAMINFO
                                                 // block.
                        }
                        ...
                        if (drflac__read_streaminfo(onRead, pUserData, &streaminfo))
                        {
                            ...
                            pInit->oggFirstBytePos =
                                pInit->runningFilePos - 79; // Subtracting 79 will place us right on top of
                                                            // the "OggS" identifier of the FLAC bos page.
    ...
    // If we get here it means we found a FLAC audio stream. We should be sitting on the first byte of the
    // header of the next page. The next packets in the FLAC logical stream contain the metadata. The only
    // thing left to do in the initialization phase for Ogg is to create the Ogg bistream object.
    pInit->hasMetadataBlocks = DRFLAC_TRUE; // <-- Always have at least VORBIS_COMMENT metadata block.
    return DRFLAC_TRUE;
}
#endif

drflac_bool32 drflac__init_private(drflac_init_info* pInit, drflac_read_proc onRead, drflac_seek_proc onSeek,
...
drflac* drflac_open_with_metadata_private(drflac_read_proc onRead, drflac_seek_proc onSeek,
                                          drflac_meta_proc onMeta, drflac_container container,
                                          void* pUserData, void* pUserDataMD)
{
    ...
    //
    // The complicated part of the allocation is making sure there's enough room the decoded samples, taking
    // into consideration the different SIMD instruction sets.
    drflac_uint32 allocationSize = sizeof(drflac);

    // The allocation size for decoded frames depends on the number of 32-bit integers that fit inside the
    // largest SIMD vector we are supporting.
    drflac_uint32 wholeSIMDVectorCountPerChannel;
    if ((init.maxBlockSize % (DRFLAC_MAX_SIMD_VECTOR_SIZE / sizeof(drflac_int32))) == 0)
    {
        wholeSIMDVectorCountPerChannel =
            (init.maxBlockSize / (DRFLAC_MAX_SIMD_VECTOR_SIZE / sizeof(drflac_int32)));
    }
    else
    {
        wholeSIMDVectorCountPerChannel =
            (init.maxBlockSize / (DRFLAC_MAX_SIMD_VECTOR_SIZE / sizeof(drflac_int32))) + 1;
    }
    ...
    // This part is a bit awkward. We need to load the seektable so that it can be referenced in-memory, but I
    // want the drflac object to consist of only a single heap allocation. To this, the size of the seek table
    // needs to be known, which we determine when reading and decoding the metadata.
    ...
    // NOTE: Seektables are not currently compatible with Ogg encapsulation (Ogg has its own accelerated
        ...
        if (seektablePos != 0)
        {
            ...
            if (pFlac->bs.onSeek(pFlac->bs.pUserData, (int)seektablePos, drflac_seek_origin_start))
            {
                if (pFlac->bs.onRead(pFlac->bs.pUserData, pFlac->pSeekpoints, seektableSize) == seektableSize)
                {
                    ...
                    for (drflac_uint32 iSeekpoint = 0; iSeekpoint < pFlac->seekpointCount; ++iSeekpoint)
                    {
                        pFlac->pSeekpoints[iSeekpoint].firstSample =
                            drflac__be2host_64(pFlac->pSeekpoints[iSeekpoint].firstSample);
                        pFlac->pSeekpoints[iSeekpoint].frameOffset =
                            drflac__be2host_64(pFlac->pSeekpoints[iSeekpoint].frameOffset);
                        pFlac->pSeekpoints[iSeekpoint].sampleCount =
                            drflac__be2host_16(pFlac->pSeekpoints[iSeekpoint].sampleCount);
                    }
                }
                else
                {
                ...
                if (!pFlac->bs.onSeek(pFlac->bs.pUserData, (int)pFlac->firstFramePos,
                                      drflac_seek_origin_start))
                {
                    DRFLAC_FREE(pFlac);
                    return NULL;
    ...
    // If we get here, but don't have a STREAMINFO block, it means we've opened the stream in relaxed mode and
    // need to decode the first frame.
...
drflac* drflac_open_file_with_metadata(const char* filename, drflac_meta_proc onMeta, void* pUserData)
{
    ...
    drflac* pFlac = drflac_open_with_metadata_private(drflac__on_read_stdio, drflac__on_seek_stdio, onMeta,
                                                      drflac_container_unknown, (void*)file, pUserData);
    if (pFlac == NULL)
    {
        fclose(file);
        return pFlac;
                                         ...
                                         void* pUserData)
{
    ...
    memoryStream.dataSize       = dataSize;
    memoryStream.currentReadPos = 0;
    drflac* pFlac = drflac_open_with_metadata_private(drflac__on_read_memory, drflac__on_seek_memory, onMeta,
...
drflac* drflac_open(drflac_read_proc onRead, drflac_seek_proc onSeek, void* pUserData)
{
    return drflac_open_with_metadata_private(onRead, onSeek, NULL, drflac_container_unknown, pUserData,
                                             pUserData);
}
drflac* drflac_open_relaxed(drflac_read_proc onRead, drflac_seek_proc onSeek, drflac_container container,
                            void* pUserData)
...
drflac* drflac_open_with_metadata(drflac_read_proc onRead, drflac_seek_proc onSeek, drflac_meta_proc onMeta,
                                  void* pUserData)
{
    return drflac_open_with_metadata_private(onRead, onSeek, onMeta, drflac_container_unknown, pUserData,
                                             pUserData);
}
drflac* drflac_open_with_metadata_relaxed(drflac_read_proc onRead, drflac_seek_proc onSeek,
                                          drflac_meta_proc onMeta, drflac_container container,
...
drflac_uint64 drflac__read_s32__misaligned(drflac* pFlac, drflac_uint64 samplesToRead,
                                           drflac_int32* bufferOut)
{
    unsigned int channelCount =
        drflac__get_channel_count_from_channel_assignment(pFlac->currentFrame.header.channelAssignment);
    ...
    drflac_assert(pFlac->currentFrame.samplesRemaining > 0 &&
    ...
    while (samplesToRead > 0)
    {
        ...
        case DRFLAC_CHANNEL_ASSIGNMENT_LEFT_SIDE:
        {
            ...
            {
                decodedSample =
                    pFlac->currentFrame.subframes[channelIndex + 0].pDecodedSamples[nextSampleInFrame]
                    << pFlac->currentFrame.subframes[channelIndex + 0].wastedBitsPerSample;
            }
            else
            {
                ...
                int left = pFlac->currentFrame.subframes[channelIndex - 1].pDecodedSamples[nextSampleInFrame]
                           << pFlac->currentFrame.subframes[channelIndex - 1].wastedBitsPerSample;
        ...
        default:
        {
            decodedSample = pFlac->currentFrame.subframes[channelIndex + 0].pDecodedSamples[nextSampleInFrame]
                            << pFlac->currentFrame.subframes[channelIndex + 0].wastedBitsPerSample;
        }
        break;
        }
    ...
    while (samplesToRead > 0)
    {
        ...
        else
        {
            if (pFlac->currentFrame.samplesRemaining > samplesToRead)
            {
                ...
                pFlac->currentFrame.samplesRemaining -= (drflac_uint32)
                    samplesToRead; // <-- Safe cast. Will always be < currentFrame.samplesRemaining < 65536.
                samplesToRead = 0;
            }
            else
            {
    ...
    while (samplesToRead > 0)
    {
        ...
        else
        {
            ...
            unsigned int channelCount = drflac__get_channel_count_from_channel_assignment(
                pFlac->currentFrame.header.channelAssignment);
            drflac_uint64 totalSamplesInFrame = pFlac->currentFrame.header.blockSize * channelCount;
            drflac_uint64 samplesReadFromFrameSoFar =
                totalSamplesInFrame - pFlac->currentFrame.samplesRemaining;
            ...
            case DRFLAC_CHANNEL_ASSIGNMENT_LEFT_SIDE:
            {
                ...
                {
                    int left  = pDecodedSamples0[i] << (unusedBitsPerSample +
                                                       pFlac->currentFrame.subframes[0].wastedBitsPerSample);
                    int side  = pDecodedSamples1[i] << (unusedBitsPerSample +
                                                       pFlac->currentFrame.subframes[1].wastedBitsPerSample);
                    int right = left - side;
            ...
            case DRFLAC_CHANNEL_ASSIGNMENT_RIGHT_SIDE:
            {
                ...
                for (drflac_uint64 i = 0; i < alignedSampleCountPerChannel; ++i)
                {
                    int side  = pDecodedSamples0[i] << (unusedBitsPerSample +
                                                       pFlac->currentFrame.subframes[0].wastedBitsPerSample);
                    int right = pDecodedSamples1[i] << (unusedBitsPerSample +
                                                        pFlac->currentFrame.subframes[1].wastedBitsPerSample);
                    int left  = right + side;
                    ...
                    for (drflac_uint64 i = 0; i < alignedSampleCountPerChannel; ++i)
                    {
                        for (unsigned int j = 0; j < channelCount; ++j)
                        {
                            bufferOut[(i * channelCount) + j] =
                                (pFlac->currentFrame.subframes[j]
                                     .pDecodedSamples[firstAlignedSampleInFrame + i])
                                << (unusedBitsPerSample +
                                    pFlac->currentFrame.subframes[j].wastedBitsPerSample);
                        }
                    }
                }
            }
...
drflac_bool32 drflac_seek_to_sample(drflac* pFlac, drflac_uint64 sampleIndex)
{
    ...
    // If we don't know where the first frame begins then we can't seek. This will happen when the STREAMINFO
    // block was not present when the decoder was opened.
    ...
    else
    {
        ...
        else
        {
            ...
            drflac_uint32 currentFrameSampleCount =
                pFlac->currentFrame.header.blockSize * drflac__get_channel_count_from_channel_assignment(
                                                           pFlac->currentFrame.header.channelAssignment);
            drflac_uint32 currentFrameSamplesConsumed =
                (drflac_uint32)(currentFrameSampleCount - pFlac->currentFrame.samplesRemaining);
...
#endif
        {
            // First try seeking via the seek table. If this fails, fall back to a brute force seek which is
            // much slower.
...
#define DRFLAC_DEFINE_FULL_DECODE_AND_CLOSE(extension, type)                                                 \
    static type* drflac__full_decode_and_close_##extension(drflac* pFlac, unsigned int* channelsOut,         \
                                                           unsigned int* sampleRateOut,                      \
                                                           drflac_uint64* totalSampleCountOut)               \
    {                                                                                                        \
        drflac_assert(pFlac != NULL);                                                                        \
                                                                                                             \
            ...
            {                                                                                                \
                ...
                {                                                                                            \
                    ...
                    pSampleData = pNewSampleData;                                                            \
                                                                                                             ...
                                                                                                             \
                drflac_copy_memory(pSampleData + totalSampleCount, buffer,                                   \
                                   (size_t)(samplesRead * sizeof(type)));                                    \
                totalSampleCount += samplesRead;                                                             \
            }                                                                                                \
                                                                                                             \
            /* At this point everything should be decoded, but we just want to fill the unused part buffer   \
               with silence - need to                                       \                                \
        ...
        {                                                                                                    \
            ...
            {                                                                                                \
                goto on_error; /* The decoded data is too big. */                                            \
                                                                                                             ...
                                                                                                             \
            pSampleData =                                                                                    \
                (type*)DRFLAC_MALLOC((size_t)dataSize); /* <-- Safe cast as per the check above. */          \
            if (pSampleData == NULL)                                                                         \
            {                                                                                                \
                goto on_error;                                                                               \
            }                                                                                                \
...
drflac_int32* drflac_open_and_decode_s32(drflac_read_proc onRead, drflac_seek_proc onSeek, void* pUserData,
                                         unsigned int* channels, unsigned int* sampleRate,
                                         drflac_uint64* totalSampleCount)
{
    // Safety.
...
drflac_int16* drflac_open_and_decode_s16(drflac_read_proc onRead, drflac_seek_proc onSeek, void* pUserData,
                                         unsigned int* channels, unsigned int* sampleRate,
                                         drflac_uint64* totalSampleCount)
{
    // Safety.
...
float* drflac_open_and_decode_file_f32(const char* filename, unsigned int* channels, unsigned int* sampleRate,
                                       drflac_uint64* totalSampleCount)
{
    if (sampleRate)
        *sampleRate = 0;
...
void drflac_init_vorbis_comment_iterator(drflac_vorbis_comment_iterator* pIter, drflac_uint32 commentCount,
                                         const void* pComments)
{
    if (pIter == NULL)
    {
                                         ...
                                         drflac_cuesheet_track* pCuesheetTrack)
{
    ...
    pRunningData += 4;
    drflac_uint64 offsetLo = drflac__be2host_32(*(const drflac_uint32*)pRunningData);
    pRunningData += 4;
...
//
// v0.10.0 - 2018-09-11
//   - Remove the DR_FLAC_NO_WIN32_IO option and the Win32 file IO functionality. If you need to use Win32
//   file IO you
//     need to do it yourself via the callback API.
//   - Fix the clang build.
//   - Fix undefined behavior.
//   - Fix errors with CUESHEET metdata blocks.
//   - Add an API for iterating over each cuesheet track in the CUESHEET metadata block. This works the same
//   way as the
//     Vorbis comment API.
...
//
// v0.8e - 2018-02-01
//   - Fix a crash when the block size of a frame is larger than the maximum block size defined by the FLAC
//   stream.
...
//
// v0.6 - 2017-07-22
//   - Add support for recovering from invalid frames. With this change, dr_flac will simply skip over invalid
//   frames as if they
//     never existed. Frames are checked against their sync code, the CRC-8 of the frame header and the CRC-16
//     of the whole frame.
//
// v0.5 - 2017-07-16
//   - Fix typos.
...
//   - API CHANGE: Swap the order of "channels" and "sampleRate" parameters in drflac_open_and_decode*().
