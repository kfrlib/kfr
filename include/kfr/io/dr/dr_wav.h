...
//     size_t numberOfSamplesActuallyDecoded = drwav_read_s32(&wav, wav.totalSampleCount,
...
//     float* pSampleData = drwav_open_and_read_file_s32("my_song.wav", &channels, &sampleRate,
...
// this, look at drwav_open_write(), drwav_open_file_write(), etc. Use drwav_write() to write samples, or
...
// - The default read function does not do any data conversion. Use drwav_read_f32() to read and convert audio
// data
//   to IEEE 32-bit floating point samples, drwav_read_s32() to read samples as signed 32-bit PCM and
//   drwav_read_s16() to read samples as signed 16-bit PCM. Tested and supported internal formats include the
//   following:
//   - Unsigned 8-bit PCM
//   - Signed 12-bit PCM
//   - Signed 16-bit PCM
...
#define DR_WAVE_FORMAT_EXTENSIBLE 0xFFFE
    ...
    typedef struct
    {
        ...
        // The number of channels making up the audio data. When this is set to 1 it is mono, 2 is stereo,
        // etc.
        ...
        // Average bytes per second. You probably don't need this, but it's left here for informational
        // purposes.
        ...
        // information about exactly how many bits a valid per sample. Mainly used for informational purposes.
    ...
    typedef struct
    {
        ...
        // The total number of samples making up the audio data. Use <totalSampleCount> * <bytesPerSample> to
        ...
        // Only used in sequential write mode. Keeps track of the desired size of the "data" chunk at the
        // point of initialization time. Always set to 0 for non-sequential writes and when the drwav object
        // is opened in read mode. Used for validation.
        ...
        struct
        {
            drwav_uint64 iCurrentSample; // The index of the next sample that will be read by drwav_read_*().
                                         // This is used with "totalSampleCount" to ensure we don't read
    ...
    //
    // If you want dr_wav to manage the memory allocation for you, consider using drwav_open() instead. This
    // will allocate a drwav object on the heap and return a pointer to it.
    //
    ...
    // dr_wav to perform a post-processing step for storing the total sample count and the size of the data
    // chunk which requires a backwards seek.
    //
    // If you want dr_wav to manage the memory allocation for you, consider using drwav_open() instead. This
    // will allocate a drwav object on the heap and return a pointer to it.
    //
    ...
    //
    // You can also use drwav_open_file() and drwav_open_memory() to open the stream from a file or from a
    // block of memory respectively.
    //
    // This is different from drwav_init() in that it will allocate the drwav object for you via
    ...
    // Close the loader with drwav_close().
    //
    // You can also use drwav_open_file_write() and drwav_open_memory_write() to open the stream from a file
    ...
    drwav* drwav_open_write(const drwav_data_format* pFormat, drwav_write_proc onWrite,
                            drwav_seek_proc onSeek, void* pUserData);
...
//// Conversion Utilities ////
    ...
    // Low-level function for converting IEEE 64-bit floating point samples to IEEE 32-bit floating point
    // samples.
    ...
    // drwav objects because the operating system may restrict the number of file handles an application can
    ...
    // for the lifetime of the drwav object.
    //
    ...
    drwav_int16* drwav_open_and_read_file_s16(const char* filename, unsigned int* channels,
    ...
    float* drwav_open_and_read_file_f32(const char* filename, unsigned int* channels,
                                        unsigned int* sampleRate, drwav_uint64* totalSampleCount);
    drwav_int32* drwav_open_and_read_file_s32(const char* filename, unsigned int* channels,
                                              unsigned int* sampleRate, drwav_uint64* totalSampleCount);
#endif
...
static const drwav_uint8 drwavGUID_W64_JUNK[16] = {
    0x6A, 0x75, 0x6E, 0x6B, 0xF3, 0xAC, 0xD3, 0x11, 0x8C, 0xD1, 0x00, 0xC0, 0x4F, 0x8E, 0xDB, 0x8A
...
drwav_bool32 drwav_init_write__internal(drwav* pWav, const drwav_data_format* pFormat,
...
drwav_bool32 drwav_init_file_write__internal(drwav* pWav, const char* filename,
...
drwav_bool32 drwav_init(drwav* pWav, drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData)
{
    ...
    else
    {
        ...
        if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM)
        {
            ...
            pWav->totalSampleCount  = (blockCount * (fmt.blockAlign - (6 * pWav->channels))) *
    ...
    // The way we calculate the bytes per sample does not make sense for compressed formats so we just set it
    // to 0.
                                  ...
                                  void* pUserData)
{
    ...
    if (!drwav_init_write__internal(pWav, pFormat, totalSampleCount, isSequential, onWrite, onSeek,
                                    pUserData))
    {
        DRWAV_FREE(pWav);
        return NULL;
