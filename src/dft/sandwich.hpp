                           ...
                           const complex<typename traits::type>* twiddle)
{
    ...
    //     // align to cache line
    //     twiddle =
    //         (const complex<T>*)(((uintptr_t)twiddle + KFR_CACHE_LINE_SIZE - 1) & ~(KFR_CACHE_LINE_SIZE -
    //         1));
    //     complex<T>* io = inout + r1;
    //     for (size_t i = 1; i < r2; ++i)
    //     {
    //         constexpr size_t w = 1ull << cfg.l2mulwidth;
    //         for (size_t j = 0; j < r1; j += w)
    //         {
    //             cvec<T, w> v = cread<w>(io + j);
    //             v            = cmuli<inverse>(cfalse, v, cread<w>(twiddle));
    //             cwrite<w>(io + j, v);
    //             twiddle += w;
    //         }
    //         io += r1;
