...
The [convolution](../advanced/dsp_glossary.md#convolution) filter efficiently computes the convolution of two signals.
The efficiency is achieved by employing the [FFT](../advanced/dsp_glossary.md#dft-vs-fft) and the circular convolution
...
convolution of a streaming input signal with a long [FIR](../advanced/dsp_glossary.md#fir-finite-impulse-response) filter $h[n]$ (where
...
When a finite stream ends, overlap-add state can still contain the response to
earlier input. Preserve that state and call [[`filter<T>::apply_zeros`:nosig]]
for $M - 1$ samples, where $M$ is the impulse-response length. See
[Convolution reverb](conv_reverb.md) for a complete example.
